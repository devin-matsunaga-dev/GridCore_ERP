...
## Current position
- **Phase:** 0 — Foundation
- **Current WP:** WP-0.7 (CI + FAST test harness)
- **Current branch:** feat/wp-0.6-react-shell (WP-0.6 complete, awaiting squash-merge)
- **Last tag:** —
...
- **`web/` exists now (WP-0.6)** — the fast loop's `npm --prefix web run test -- --run` line is live, and the AppHost's Vite resource is no longer skipped. `npm ci` in `web/` is a prerequisite of the fast loop; **WP-0.7 must add it to CI** alongside a Node 24 setup step.
...
- TypeScript is **7.x** (the native compiler): `baseUrl` is removed, so path aliases in `tsconfig.app.json` are relative to the file. `npm run lint` is `tsc -b --noEmit`; **no ESLint is configured yet** — WP-0.7's call whether to add one.
...
- Vitest runs 124 tests in ~3.0s; the whole fast loop (dotnet build + 218 .NET unit tests + Vitest) is **~11s**.
