...
## Current position
- **Phase:** 1 — Registries
- **Current WP:** **WP-1.1 complete, awaiting owner verification and squash-merge.** Next is WP-1.2 (Service accounts + lifecycle); branch `feat/wp-1.2-service-accounts` off main
- **Current branch:** `feat/wp-1.1-customers` (off `3e690d0`)
...
- **The Customers module is live (WP-1.1)** — `customers.customers` and `customers.service_locations`, migration `WP11_Customers`, endpoints under `/api/customers` and `/api/service-locations`, all gated on `customers.read` / `customers.write` (no new permissions; only Administrator and CustomerService hold write). **WP-1.2 owns the service account** that joins the two registries — nothing joins them today, on purpose.
...
- **There is no DELETE in the registry, and there should not be one in any later registry either.** A premise is deactivated (`IsActive` + reason, recorded only when the flag actually moves), a customer is Closed. Meters, bills and work orders reference these rows. A fast test asserts no route maps DELETE.
- The customer status machine is `Prospect → Active ⇄ Suspended → Closed`, `Closed` terminal, in `CustomerTransitions`. It is the **customer record's** status, not the service account's — WP-1.2 adds Active/Pending/Disconnected/Closed on the account. An illegal move is a **409 from the aggregate**, never a 400: legality depends on current state, which a validator cannot see.
- `CustomerRegistered` and `ServiceLocationRegistered` are published through the outbox and **nothing consumes them yet** (owner's call). WP-1.2/2.x can consume them with no change in Customers. Both carry primitives only — class and status travel as names.
...
- **A module that persists anything now owes four things:** its `<Name>DbContext` (with `SchemaName`), an `IDesignTimeDbContextFactory` (it is a class library), `services.AddGridCoreDbContext<T>((b, c) => b.UseNpgsql(c, GridCoreDbContexts.InSchema(T.SchemaName)))` in `AddServices`, and a line in `GateFixture.CreateContexts()`. `Microsoft.EntityFrameworkCore.Design` goes in the csproj as a `PrivateAssets=all` reference. Copy `src/Modules/Inventory` for the smallest example, or **`src/Modules/Customers` for a complete one** (context, services, validators, endpoints, seeder, tests).
- **Migrations at startup are `GridCoreDatabaseInitializer`** (renamed from `PlatformDatabaseInitializer`) and it migrates **every** registered context, discovered through the `UnitOfWorkParticipant` registrations. Still Development-only; **WP-5.1 still owes a production migration step**, and it now has five schemas to apply, not one.
...
- Two seeders today: `ApprovalQueueDemoSeeder` (order 100 — two pending approvals, attributed to `DemoActor`s with `demo:` ids so a signed-in Manager can actually decide them, since the aggregate refuses a decision by the requester) and `CustomersDemoSeeder` (order 200). **WP-1.2 onwards each add their own**, in dependency order.
...
- **A fresh database logs four scary-looking `fail:` lines at startup, and they are harmless.** `NpgsqlHistoryRepository.Exists()` returns **true** on a database with no history table, so EF goes ahead and runs `SELECT "MigrationId" … FROM <schema>.__ef_migrations_history`, logs the failure at Error, then migrates correctly anyway. Verified against a fresh Postgres: all four schemas migrate, all reference data lands (15 accounts / 2 plans / 5 tiers / 3 warehouses), and a second start is completely silent. This is Npgsql behaviour, not ours, and it predates WP-0.8 — the platform context has always done it; WP-0.8 just made it four lines instead of one. Do **not** "fix" it by filtering the EF command-error log category, which would hide real query failures.
...
- Vitest runs 124 tests in ~2.8s; the whole fast loop (dotnet build + **434** .NET unit tests + Vitest) is **~13.4s** as of WP-1.1. **Measured at the Phase 0 gate:** the full suite is 13 assemblies / 325 .NET tests (315 unit + 10 integration) in **14.2s** plus Vitest's 18 files / 124 tests in **3.1s** — ~17s wall for everything, three containers started from cold included.
