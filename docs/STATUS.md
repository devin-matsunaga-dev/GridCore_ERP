...
## Current position
- **Phase:** 0 — Foundation
- **Current WP:** none — **Phase 0 gate passed and tagged `v0.1-phase0`.** Next is WP-1.1 (Customers & service locations); branch `feat/wp-1.1-customers` off main
- **Current branch:** main (clean, in sync with origin; WP-0.8 squash-merged as `daba607`)
...
- **A module that persists anything now owes four things:** its `<Name>DbContext` (with `SchemaName`), an `IDesignTimeDbContextFactory` (it is a class library), `services.AddGridCoreDbContext<T>((b, c) => b.UseNpgsql(c, GridCoreDbContexts.InSchema(T.SchemaName)))` in `AddServices`, and a line in `GateFixture.CreateContexts()`. `Microsoft.EntityFrameworkCore.Design` goes in the csproj as a `PrivateAssets=all` reference. Copy `src/Modules/Inventory` — it is the smallest of the three.
- **Migrations at startup are `GridCoreDatabaseInitializer`** (renamed from `PlatformDatabaseInitializer`) and it migrates **every** registered context, discovered through the `UnitOfWorkParticipant` registrations. Still Development-only; **WP-5.1 still owes a production migration step**, and it now has four schemas to apply, not one.
...
- The only seeder today is `ApprovalQueueDemoSeeder` (two pending approvals, attributed to `DemoActor`s with `demo:` ids so a signed-in Manager can actually decide them — the aggregate refuses a decision by the requester). **WP-1.x/2.x/3.x each add their own seeder** for the entities SPEC.md lists; nothing else has entities yet.
...
- Vitest runs 124 tests in ~2.8s; the whole fast loop (dotnet build + 315 .NET unit tests + Vitest) is **~13s**. **Measured at the Phase 0 gate:** the full suite is 13 assemblies / 325 .NET tests (315 unit + 10 integration) in **14.2s** plus Vitest's 18 files / 124 tests in **3.1s** — ~17s wall for everything, three containers started from cold included.
