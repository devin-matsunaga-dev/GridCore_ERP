...
## Current position
- **Phase:** 0 — Foundation
- **Current WP:** none — WP-0.8 complete. Next is the **Phase 0 gate** (fresh clone → `aspire run` → login → outbox round-trip, full gate suite, `aspire update`, tag `v0.1-phase0`), then WP-1.1 (Customers & service locations)
- **Current branch:** feat/wp-0.8-reference-data-seed (WP-0.8 complete; awaiting review and squash-merge)
- **Last tag:** —
...
- Vitest runs 124 tests in ~3.6s; the whole fast loop (dotnet build + 315 .NET unit tests + Vitest) is **~13s**. The full gate suite (10 tests, three containers started from cold) is **~12.5s** — measured, not estimated.
...
- [x] WP-0.8 Reference data + demo seeder (split)   → gate v0.1-phase0
