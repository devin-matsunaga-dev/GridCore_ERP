...
/// <summary>
/// The metering schema and the platform schema on one SQLite in-memory connection — the fast-tier
/// equivalent of the shared Postgres connection the host gives a request. That is what lets these
/// tests assert the thing that actually matters about a register write: the meter row, its history
/// line, its audit entry and its event all belong to one transaction (CONVENTIONS.md rule C).
/// </summary>
/// <remarks>
/// The customers schema is deliberately <b>absent</b>. Metering reads premises through
/// <see cref="IServiceLocationDirectory"/>, so the fake stands in for the whole of another module
/// and its database — which is the point of the seam, and the reason a meter assignment can be
/// tested in milliseconds.
/// </remarks>
public sealed class MeteringTestHost : IDisposable
{
    ...
    public MeteringTestHost(TimeProvider? clock = null, ICurrentUser? currentUser = null)
    {
        ...
        services.AddGridCoreDbContext<MeteringDbContext>((builder, connection) => builder.UseSqlite(connection));
    ...
    /// <summary>Runs <paramref name="work"/> against the meter register, in its own scope.</summary>
    public Task<TResult> WithMetersAsync<TResult>(Func<IMeterService, Task<TResult>> work)
    {
        ArgumentNullException.ThrowIfNull(work);

        return InScopeAsync(services => work(services.GetRequiredService<IMeterService>()));
    }
    ...
    public MeteringDbContext NewMeteringContext() =>
        new(new DbContextOptionsBuilder<MeteringDbContext>().UseSqlite(_connection).Options);
    ...
    private void CreateTables()
    {
        ...
        scope.ServiceProvider.GetRequiredService<MeteringDbContext>()
            .Database.GetService<IRelationalDatabaseCreator>().CreateTables();
    }
}
